...
import { Plus, X, Save, ThumbsUp, ThumbsDown, ListChecks } from 'lucide-react'
...
export default function AturanKelasPage() {
  ...
  function resetForm() {
    setJudul(''); setJenisPoin('positif'); setPoinAngka('1'); setKategori('mandiri')
  }
  ...
  async function handleSave() {
    ...
    const { data: { user } } = await supabase.auth.getUser()
    await supabase.from('aturan_kelas').insert({
      kelas_id: kelas.id, judul: judul.trim(), poin: poinFinal, kategori, dibuat_oleh: user.id
    })
    setSaving(false)
    setShowForm(false)
    resetForm()
      ...
      <main className="pt-14 pb-8">
        ...
        <div className="bg-white border-b border-gray-100 flex">
          ...
          <button onClick={() => setTab('negatif')}
            ...
            <ThumbsDown size={16} /> Perlu Diperbaiki ({aturanNegatif.length})
          </button>
        </div>
            ...
            <div className="bg-white w-full rounded-t-2xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setJenisPoin('negatif')}
                  ...
                  <ThumbsDown size={14} /> Perlu Diperbaiki
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Nilai Poin</label>
                  <input type="number" min="1" max="10" className="input" value={poinAngka} onChange={e => setPoinAngka(e.target.value)} />
                </div>
        ...
        <div className="px-4 mt-4 space-y-2">
          {tampil.length === 0 && (
            <div className="card text-center py-8">
              ...
              <p className="text-gray-400 text-sm">Belum ada aturan {tab === 'positif' ? 'positif' : 'perlu diperbaiki'}.</p>
            </div>
          )}
          {tampil.map(a => (
